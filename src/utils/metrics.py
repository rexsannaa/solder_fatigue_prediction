...
def calculate_relative_error(y_true, y_pred, percentile=None):
    ...
    elif not isinstance(percentile, (list, tuple, np.ndarray)):
...
def plot_metrics_comparison(models_results, metrics=None, figsize=(12, 8)):
    ...
    for i, metric in enumerate(metrics):
        if metric in models_results.columns:
            ...
            bars = axes[i].bar(
                data['model'] if 'model' in data.columns else data.index,
                data[metric],
                color=colors
            )
            for bar in bars:
...
if __name__ == "__main__":
    ...
    # 創建測試數據
    np.random.seed(42)
    y_true = np.exp(np.random.normal(10, 2, size=50))  # 假設真實壽命數據
    noise = np.random.normal(0, 0.2, size=50)          # 假設雜訊
    y_pred = y_true * (1 + noise)
    # 評估模型
    results = evaluate_model(y_true, y_pred, model_name="TestModel", verbose=True)
    print(results)
