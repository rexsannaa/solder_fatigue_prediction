...
class HybridLoss(nn.Module):
    ...
    def forward(self, outputs, targets, model=None):
        ...
        total_loss = (
            ...
            delta_w_weight * 2.0 * delta_w_combined_loss +  # 更強化delta_w預測權重
            lambda_consistency * 0.5 * delta_w_consistency +  # 調整分支一致性損失
            lambda_physics * 3.0 * physics_loss +  # 大幅增強物理約束損失
            1.5 * direct_delta_w_loss +  # 進一步增強直接delta_w引導損失
            reg_loss  # 正則化損失
        )
